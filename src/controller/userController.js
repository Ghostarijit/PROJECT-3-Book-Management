...
const createuser = async (req, res) => {
    try {
        ...
        if (name === undefined) return res.status(400).send({ status: false, msg: "name must be present" });
        ...
        if (!pass) return res.status(400).send({ status: false, msg: "1.At least one digit, 2.At least one lowercase character,3.At least one uppercase character,4.At least one special character, 5. At least 8 characters in length, but no more than 15" })
